...
- [ ] **Runtime cutover (remaining):** point the app's request-path connection at `DATABASE_URL_APP` (the `phila_app` role) and set the org GUCs per request inside a `sql.transaction([...])` (the mechanism the proof test exercises). Until then RLS is a proven, enforced *DB-level* boundary; the running app's primary boundary stays the app-layer `where org_id = …`. The flip needs the read/write paths wrapped in an org-context transaction (neon-http is per-request stateless), so it's a deliberate refactor, not a config toggle.
