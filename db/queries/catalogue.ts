...
/** Replace the org's service catalogue with `list` (upsert kept, delete removed). */
...
/** Upsert a single room (create with a real id, or update an existing one). */
export async function saveRoom(orgId: string, room: { id?: string; name: string; siteId: string; capacity: number; equipment: string[]; status: string; colour: string }): Promise<void> {
  ...
  await db.insert(roomsTable).values(values).onConflictDoUpdate({ target: roomsTable.id, set: { siteId: values.siteId, name: values.name, capacity: values.capacity, equipment: values.equipment, status: values.status, colour: values.colour } });
}

/** Replace the org's sites with `list` (upsert kept, delete removed). */
