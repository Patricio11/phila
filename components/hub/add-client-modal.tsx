...
export function AddClientButton({ counsellors }: { counsellors: { id: string; name: string }[] }) {
  ...
  const submit = () => {
    setAttempted(true);
    if (errors.name || errors.counsellor || errors.phone || errors.email) return;
    start(async () => {
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label>Phone</Label>
              <Input inputMode="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="082 123 4567" invalid={Boolean(attempted && errors.phone)} />
              {attempted && errors.phone ? <FieldError>{errors.phone}</FieldError> : null}
            </div>
            <div className="space-y-1.5">
              <Label>Email</Label>
              <Input inputMode="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="optional" invalid={Boolean(attempted && errors.email)} />
