...
export async function createPayment(orgId: string, pack: CreditPack, provider: string, providerRef: string): Promise<void> {
  await getDb().insert(payments).values({
    ...
    purpose: pack.channel === "sms" ? "credit_sms" : pack.channel === "video" ? "credit_video" : "credit_email",
 ...
 */
export async function settlePayment(providerRef: string): Promise<{ credited: number; channel: "sms" | "email" | "video" | null }> {
  const db = getDb();
  ...
  const channel = pay.purpose === "credit_sms" ? ("sms" as const) : pay.purpose === "credit_video" ? ("video" as const) : ("email" as const);
