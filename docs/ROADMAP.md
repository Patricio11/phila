...
- [x] **Appointment detail (2026-06-27):** clicking a calendar event opens a calm `<AppointmentDetail>` card — client (linked), status, full date + time range, duration, counsellor, room/online — with **View client** and (counsellor) **Open session** actions, instead of jumping straight to the note.
